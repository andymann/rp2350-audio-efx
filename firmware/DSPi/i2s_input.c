 ...
 *
 * Single-pair capture, always running the plain wait-driven "slave" PIO
 * program (audio_i2s_rx_slave in i2s_input.pio) against this device's
 * OWN I2S output clock. IRQ-less self-retriggering DMA ring, same
...
// 1024 words = 512 stereo frames = ~10.7ms of headroom @ 48kHz, matching
...
static uint16_t slave_prog_ram[
    sizeof(audio_i2s_rx_slave_program_instructions) / sizeof(uint16_t)];
...
void i2s_input_init(void)
{
    ...
    // Load + patch the slave program.
    uint8_t len = audio_i2s_rx_slave_program.length;
    memcpy(slave_prog_ram, audio_i2s_rx_slave_program_instructions,
    ...
    // BCK/LRCLK are inputs here -- i2s_output.c already configured them
    // as PIO0 outputs; this SM only samples them, no pindirs to set.

    audio_i2s_rx_slave_program_init(I2S_IN_PIO, I2S_IN_SM, offset, I2S_DATA_IN_PIN);
